...
public class ComerciosFragment extends Fragment implements SearchView.OnQueryTextListener{
    private final String TAG = ComerciosFragment.class.getSimpleName();

    private LinearLayoutManager linearLayoutManager;
    private RecyclerView reciclador;
    private List<Comercio> items = new ArrayList<>();
    private ItemAdapter itemAdapter;
    private SwipeRefreshLayout mSwipeRefreshLayout;
    private TextView tvMensaje;
    private ProgressBar pBar;
    private List<Comercio> filteredModelList = new ArrayList<>();
    private DBHelper db;
    ...
    public View onCreateView(LayoutInflater inflater,
        ...
        View v = inflater.inflate(R.layout.comercios_fragment, container, false);
    ...
    public boolean onQueryTextChange(String query) {
        if (!items.isEmpty()) {
            filteredModelList = filter(items, query);
            ArrayList<Object> models = new ArrayList<>();
            models.addAll(filteredModelList);
            itemAdapter.animateTo(models);
            reciclador.scrollToPosition(0);
    ...
    private List<Comercio> filter(List<Comercio> models, String query) {
        query = query.toLowerCase();

        final List<Comercio> filteredModelList = new ArrayList<>();
        for (Comercio model : models) {
            StringBuilder text = new StringBuilder();
            text.append(model.getNombreFantasia());
            text.append(model.getRazonSocial());
            if (model.getRubro() != null){
                text.append(model.getRubro().getNombre());
            }
            if (model.getSubrubro() != null){
                text.append(model.getSubrubro().getNombre());
            }
            text.append(model.getDescripcion());
            String textoBusqueda = Operacion.quitarTildes(text.toString().toLowerCase());
            if (textoBusqueda.contains(query)) {
    ...
    private void mostrarComercios() {
        if (!Almacen.getInstancia().getComercios().isEmpty()){
            ocultarMensaje();
            items = Almacen.getInstancia().getComercios();
        } else {
            mostrarMensaje();
        }
        ArrayList<Object> models = new ArrayList<>();
        ...
        itemAdapter.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(filteredModelList.size() < items.size() & !filteredModelList.isEmpty()){
                    inicializarActividadComercio(filteredModelList.get(reciclador.getChildPosition(v)).getId());
                }else{
                    inicializarActividadComercio(items.get(reciclador.getChildPosition(v)).getId());
                }
            }
        });
